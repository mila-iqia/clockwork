...
def get_mongo_client(config:dict=D_mongo_config):
    ...
    if get_mongo_client.value is None:
        ...
        if 'connect_template' not in config:
            config['connect_template'] = "mongodb://<username>:<password>@<hostname>:<port>/?authSource=admin&readPreference=primary&retryWrites=true&w=majority&ssl_cert_reqs=CERT_NONE&ssl=false"

        connection_string = config['connect_template'].replace("<username>", config['username']).replace("<password>", config['password']).replace("<hostname>", config["hostname"]).replace("<port>", str(config['port']))
