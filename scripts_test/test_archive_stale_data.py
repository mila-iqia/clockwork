...
def test_archive_stale_data():
    ...
    def get_random_timestamp():
        ...
        if np.random.rand() < 0.5:
            return (now - seconds_since_last_update) - 1000*(1+np.random.randn()**2)
        else:
            return (now - seconds_since_last_update) + 1000*(1+np.random.randn()**2)
    ...
    with tempfile.NamedTemporaryFile(mode='w', dir=None, delete=False) as f_out:
        ...
        contents_archived_0 = archive_stale_data.archive(archive_path, database_name, days_since_last_update)
    ...
    assert sorted_by_last_update(contents_archived_0) == sorted_by_last_update(contents_archived_1)
    
    # Validate that the archived contents is the old stuff.
    assert (sorted_by_last_update({"jobs": LD_stale_jobs, "nodes": LD_stale_nodes})
    ...
    for D in LD_nodes_still_in_database:
        del D["_id"]
    #
    assert (sorted_by_last_update({"jobs": LD_fresh_jobs, "nodes": LD_fresh_nodes})
            == sorted_by_last_update(contents_archived_1))
