...
def route_list(cluster_name:str, reservation:str):
    ...
    if cluster_name is not None:
        if not re.match(r"^[a-zA-Z]+$", cluster_name):
            return render_template("error.html", error_msg="cluster_name specific contains invalid characters")
    ...
    if reservation is not None:
        ...
        if not re.match(r"^[a-zA-Z\.\d\-]+$", reservation):
            return render_template("error.html", error_msg="reservation specific contains invalid characters")
...
def route_single_node(node_name:str):
    ...
    if not re.match(r"^[a-zA-Z\d\-]+$", node_name):
        return render_template("error.html", error_msg="node_name specific contains invalid characters")

    LD_nodes = get_nodes({'name':node_name})
    if len(LD_nodes) == 0:
        return render_template("error.html", error_msg=f"Failed to find a node with name {node_name}.")
    elif len(LD_nodes) > 1:
        return render_template("error.html", error_msg=f"This is weird. We seem to have more than one node with name {node_name}.")
    ...
    return render_template("nodes/single_node.html",
                            node_name=node_name,
                            D_node=D_node,
                            LP_node=LP_node,
                            LD_jobs=LD_jobs)
